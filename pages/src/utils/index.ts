...
export const fetchAndParseParquet = async (path: string) => {
    const url = import.meta.env.PROD ? `${raw_base_url}${path}` : `${SERVER_URL.URL}/static/${path}`
    const file: AsyncBuffer = await asyncBufferFromUrl({ url })
...
/** 请求tif */
export const fetchTif = async (path: string) => {
    const url = import.meta.env.PROD ? `${raw_base_url}/${path}` : `${SERVER_URL.URL}/static/${path}`
    const tiff = await fetch(url).then(resp => resp.arrayBuffer()).then(arrayBuffer => GeoTIFF.fromArrayBuffer(arrayBuffer))
    return tiff
}
