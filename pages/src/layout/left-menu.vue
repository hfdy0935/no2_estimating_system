        ...
        <n-spin :show="loading" description="加载中">
            <n-tabs type="segment" animated>
                <n-tab-pane :name="MenuType.DAILY" tab="每天">
                    <n-menu v-if="dailyMenuOptions.length || loading" v-model:value="activeKey" :collapsed
                        :collapsed-width="96" :collapsed-icon-size="22" :indent="14" :options="dailyMenuOptions"
                        @update-value="(_: any, option: MenuOption) => selectedMenuOption = option" />
                ...
                <n-tab-pane :name="MenuType.HOURLUY" tab="每时">
                    <n-menu v-if="hourlyMenuOptions.length || loading" v-model:value="activeKey" :collapsed
                        :collapsed-width="96" :collapsed-icon-size="22" :indent="14" :options="hourlyMenuOptions"
                        @update-value="(_: any, option: MenuOption) => selectedMenuOption = option" />
                    <n-empty v-else description="暂无数据" :show-icon="false" style="padding-top: 240px;">
                        <template #extra>
                            <n-button size="small" @click="openRepo">
