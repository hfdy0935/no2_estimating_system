<template>
    <n-popover trigger="hover" placement="right" display-directive="show">
        ...
        <n-card style="width: 600px;padding: 0;max-height: 800px;overflow: auto;" :bordered="false">
            <template #header>
...
watch(selectedMenuOption, async () => {
    ...
    try {
        ...
        message.error(`统计数据加载失败，数据不存在或出现错误，请稍后再试是否存在或重试或联系作者:${e}`, { keepAliveOnHover: true })
    }
})
</script>
