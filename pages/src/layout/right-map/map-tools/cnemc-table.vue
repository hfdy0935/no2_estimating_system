<template>
    <n-button v-if="showedCnemcData.length" type="success" style="margin: 10px;" size="small"
        @click="downloadAsCsv">下载为csv</n-button>
    <n-data-table :columns :data="showedCnemcData" :max-height="400" virtual-scroll size="small" bordered ref="table"
        :scroll-x="600" />
</template>
...
import type { ICnemcItem } from './types';
import { useMenuStore } from '@/stores/menu';

const { selectedFilename } = storeToRefs(useMenuStore())
const { showedCnemcData } = storeToRefs(useMapStore())

const columns: DataTableColumns<ICnemcItem> = [
    ...
    { title: '质量', key: 'quality', width: 100, render: (row: ICnemcItem) => h(CnemcTableItemRender, { quality: row.quality }) },
