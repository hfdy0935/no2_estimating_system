...
export const useMapStore = defineStore('map', () => {
    ...
    const tableCnemcData = computed<ITableCnemcItem[]>(() => {
        ...
        const targetData = selectedMenuType.value === MenuType.DAILY ? cnemcData.value : { [ymdh]: cnemcData.value[ymdh] }
        let arr: Omit<ITableCnemcItem, 'index'>[] = []
        for (const el of Object.values(targetData)) {
            arr = arr.concat(el as Omit<ITableCnemcItem, 'index'>[])
        }
        return arr.map((el, i) => ({
            ...el,
            lon: +el.lon.toFixed(2),
            lat: +el.lat.toFixed(2),
            cnemc_no2: +el.cnemc_no2.toFixed(2),
            area: el.area ?? '',
            station_code: el.station_code ?? '',
    ...
    const mapCnemcData = computed<IMapCnemcItem[]>(() => {
        ...
        // 选中的小时的具体数值
        return tableCnemcData.value.map(({ quality, ...rest }) => ({ ...rest, quality: [quality] }))
    })
