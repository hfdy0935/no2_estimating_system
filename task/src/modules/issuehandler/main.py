...
available_start = (
    time_util.dt2ymd(est_record_util.dt_start)
    if est_record_util.dt_start is not None
    else '-'
)
available_end = (
    time_util.dt2ymd(est_record_util.dt_end)
    if est_record_util.dt_end is not None
    else '-'
)
...
class IssueContent:
    ...
    def get_dates_need_send(self) -> list[datetime]:
        """实际发送的那天的时间列表"""
        if self.date not in est_record_util.available_dts:
            return []
...
class Emailtool:
    ...
    def _send_one(
        ...
        Args:
            ...
            content_info (IssueContent): _description_
            dts (list[datetime]): 本次要准备的时间范围(list[ymdh])，可能文件总量过大，要分批发送
...
class IssueHandler:
    def __init__(self):
        # 请求数据的固定issue title
        self.target_issue_title: Literal['req_data'] = 'req_data'
        self.email_tool = Emailtool()
        self.issue_tool = IssueTool(self.target_issue_title)

    def _parse_issue_content(self, content: str) -> IssueContent:
        """解析请求的issue内容
