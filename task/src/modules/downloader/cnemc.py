...
from src.utils.light import time_util, path_util, df_util, logger, data_record_util, cs
...
class CNEMCDownloader:
    ...
    def _dict2dataframe(self, data: list[dict]) -> pd.DataFrame:
    ...
    def download_one(self, dt: datetime) -> pd.DataFrame:
        ...
        resp = self.session.post(
            url=self.base_url,
            data={
                'date': self._format_dt(dt + timedelta(hours=8)),
            },
        )
        data = resp.json()
        # 如果没有数据
        if len(data) == 0:
            return pd.DataFrame()
        return self._dict2dataframe(data)

    def download(self, dt: datetime):
        """下载前面24h的cnemc数据，排除已下载的小时
        dt: utc
        """
        logger.info(cs.blue('=' * 50 + time_util.dt2ymd(dt) + ' CNEMC ' + '=' * 50))
        logger.info('准备下载之前24h内的数据')
        # 已有的dt str, ymdh
        cur_dts: list[str] = []
        # 今天已有的
        cur_path = path_util.gen_pq_path_under_ds(
            [
                'cnemc',
            ],
            dt,
        )
        logger.info(f'当前的文件路径为: {cur_path}')
        if os.path.exists(cur_path):
            df = pd.read_parquet(cur_path)
            cur_dts.extend(list(df.time.unique()))
        # 昨天已有的
        pre_path = path_util.gen_pq_path_under_ds(
            [
                'cnemc',
            ],
            dt - timedelta(days=1),
        )
        logger.info(f'前一天的文件路径为: {pre_path}')
        ...
        for ymd, group in df.groupby(df['time'].str[:-2]):
            savepath = path_util.gen_pq_path_under_ds(
                ['cnemc'], dt=time_util.ymd2dt(ymd)
            )
            df_util.append_parquet(df=group, path=savepath)
            logger.info(
                cs.green(f'  √ 下载完成，已保存至{path_util.rel2abs(savepath)}')
            )


def download_cnemc(dt: Maybe[datetime] = None):
    with data_record_util.write_with_duration('CNEMC'):
        downloader = CNEMCDownloader()
        downloader.download(dt or time_util.utc_now())
