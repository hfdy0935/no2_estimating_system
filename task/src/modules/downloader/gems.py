...
class GEMSKeyFetcher(GEMSFetcher):
    """使用带key的链接请求GEMS数据"""
...
class GEMSBrowserFetcher(GEMSFetcher):
...
class GEMSDownloader:
    ...
    **数据**
    >>> 每天大约utc 00:45-05:45有数据，处理需要一个多小时
    ...
    **下载**
    ...
    def __init__(self, dt: datetime, fetcher: Literal['apikey', 'browser'] = 'browser'):
        self.fetcher: GEMSFetcher = (
            GEMSKeyFetcher() if fetcher == 'apikey' else GEMSBrowserFetcher()
        )
        self.dt = dt
    ...
    def _read_gems(self, buffer: BytesIO, ymdh: str) -> pd.DataFrame:
        ...
        df = resample_util.grid_divide_resample_v1(df=df, columns=[self.no2_column])
    ...
    def download(self):
        logger.info(cs.blue('=' * 50 + time_util.dt2ymd(self.dt) + ' GEMS ' + '=' * 50))
        ...
        for cur, item in params:
            # tasks.append(
            #     lambda cur, item: self._read_gems(
            #         self.fetcher.fetch_data(cur, item), cur[:-2]
            #     )
            # )
            # argss.append([cur, item])
            df_ls.append(self._read_gems(self.fetcher.fetch_data(cur, item), cur[:-2]))
            self.log('下载成功', cur[:-2])
        # each_callback = lambda _, cur, *__: self.log(
        #     '处理完毕', cur[:-2]
        # )  # xxx_min=>xxx_hour
        # for df in run_in_thread_pool(
        #     tasks=tasks, argss=argss, each_callback=each_callback
        # ):
        #     df_ls.append(df)
        if len(df_ls) == 0:
...
def download_gems(dt: Maybe[datetime] = None):
    with data_record_util.write_with_duration('GEMS'):
        downloader = GEMSDownloader(dt=dt or time_util.utc_now(), fetcher='apikey')
        downloader.download()
