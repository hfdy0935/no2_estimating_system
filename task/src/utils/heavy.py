...
class FilterUtil:
    ...
    """判断点是否在china区域的结果缓存，key是缓存的唯一标识，GEOS-CF网格规则，会用到"""
    ...
    def filter_china(
        ...
        # cahce_key不为None且有缓存，表示要使用缓存
        if (
            cache_key in self.filter_china_land_cache
            and (cur_cache := self.filter_china_land_cache.get(cache_key)) is not None
        ):
    ...
    """判断点是否在世界陆地区域的结果缓存，key是缓存的唯一标识，GEOS-CF网格规则，会用到"""
    ...
    def filter_world_land(
        ...
        # cahce_key不为None且有缓存，表示要使用缓存
        if (
            cache_key in self.filter_world_land_cache
            and (cur_cache := self.filter_world_land_cache.get(cache_key)) is not None
        ):
